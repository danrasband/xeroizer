  ...
  module Record
    ...
    class BaseModel
        ...
        def parse_response(response_xml, options = {})
          Response.parse(response_xml, options) do | response, elements, response_model_name |
            if model_name == response_model_name
              @response = response
              parse_records(response, elements, (options[:base_module] || Xeroizer::Record), paged_records_requested?(options))
            end
          end
        end
      ...
      protected
        ...
        def parse_records(response, elements, base_module, paged_results)
